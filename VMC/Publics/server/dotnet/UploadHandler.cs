...
namespace VMC.Publics.server.dotnet
{
    public class UploadHandler
    {
        ...
        private void PropertiesInit(string path, string url)
        {
            ...
            //</system.web>
            //this.max_file_size = 1012400000;
            this.min_file_size = 1;
            this.accept_file_types = @"^.+\.((jpg)|(gif)|(jpeg)|(png)|(mp4))$";
        ...
        //public List<UploadFileInfo> DataTabletoString()
        //{
        //    DataTable dt = (new FileModel()).File_Select(15);
        ...
        public UploadFileInfo FileUploadHandle(HttpPostedFile uploaded_file, string name, long size, string type, string error, int index, Int32 videoid, string title, Int32 ordering, Int32 typefile)
        {
            UploadFileInfo file = new UploadFileInfo();
            file.name = name;
            file.size = size;
            ...
            if (Validate(uploaded_file, file, error, index))
            {
                ...
                if (file.size == new FileInfo(file_path).Length)
                {
                    ...
                    //}
                    //insert db
                    int uid = Convert.ToInt32(Common.Common.getUserInfo().uid.ToString());
                    string uname = Common.Common.getUserInfo().uname.ToString();
                        FileModel fm = new FileModel();
                        fm.File_Insert(file.name, file.url, typefile, videoid, title, ordering, uid, uname, file.size);
